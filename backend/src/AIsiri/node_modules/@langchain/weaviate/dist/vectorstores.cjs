...
exports.WeaviateStore = exports.WeaviateDocument = exports.flattenObjectForWeaviate = void 0;
...
class WeaviateDocument extends documents_1.Document {
    constructor() {
        super(...arguments);
    ...
    async generate(query, generate, options) {
        ...
        if (this.tenant) {
            result = await collection
                .withTenant(this.tenant)
                .generate.hybrid(query, { ...(generate || {}) }, { ...(options || {}) });
        }
        else {
            result = await collection.generate.hybrid(query, { ...(generate || {}) }, { ...(options || {}) });
        }
        const documents = [];
        for (const data of result.objects) {
